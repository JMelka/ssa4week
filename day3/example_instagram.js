...
function loadImageHandler(xhttp) {
   if (xhttp.readyState === 4 && xhttp.status === 200) {
        imagesArr = JSON.parse(xhttp.responseText);
...
function renderImages(_imagesArr) {
    ...
    for(var i = 0; i < _imagesArr.length; i++) {
        ...
        listHTML += "<img src='http://localhost:8080/images/" + _imagesArr[i].url + 
            "' title='" + _imagesArr[i].description + "'>";
    }
    if(listHTML === "") {
...
function handleFilter() {
    ...
    var fiterText = document.getElementById("filterText").value;
    for(var i = 0; i < imagesArr.length; i++) {
        if(imagesArr[i].description.includes(filterText)) {
